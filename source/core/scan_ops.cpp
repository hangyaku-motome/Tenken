...
std::vector<HitInfoT> ScanOp::startScan(const Scanner &ScannerObj,
                                        SessionState &State) {
  ...
  for (uint64_t i = 0; i < Maps.size(); ++i) {
    ...
    std::vector<uint8_t> Data =
        ScannerObj.readAdr(Maps[i].start, Maps[i].end - Maps[i].start);
    ...
    dispatchType(State.TargetInfo.TargetType, [&]<typename T> {
      if constexpr (std::is_same_v<T, std::string>) { // string OR byte.
        for (const auto RelativeOffset :
             searchRawValue(Data, State.TargetInfo.value)) {
          ReturnHits.push_back(
              {.location = Maps[i].start + RelativeOffset,
               ...
               .bytes_around = findBytesAround(
                   RelativeOffset, Data,
                   static_cast<uint32_t>(State.TargetInfo.value.size()))});
        }
